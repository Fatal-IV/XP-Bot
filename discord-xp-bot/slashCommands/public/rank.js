...
module.exports = {
  ...
  async execute(interaction, client, t, db) {
    ...
    if (!member) {
        return interaction.reply({ 
            content: t('commands.rank.userNotFound'), 
            flags: MessageFlags.Ephemeral 
        });
    }
    ...
    try {
        ...
        if (!hasTextXP && !hasVoiceXP) {
            return interaction.editReply({ 
                content: t('commands.rank.noData') 
            });
        }
        ...
        await interaction.editReply({ content: t('commands.rank.cardError') });
    }
  },
};