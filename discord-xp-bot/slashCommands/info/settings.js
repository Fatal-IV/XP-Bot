...
module.exports = {
  ...
  async execute(interaction, client, t, db) {
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
       return interaction.reply({ content: t('common.noPermission'), ephemeral: true });
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { 
            name: `${logStatus} ${t('commands.settings.logChannel')}`, 
            value: `└ ${logChannelValue}`, 
            inline: true 
        },
        { 
            name: `⏱️ ${t('commands.settings.cooldown')}`, 
            value: `└ ${t('commands.settings.seconds', { sec: guildSettings.cooldown })}`, 
            inline: true 
        },
        { name: '\u200b', value: '\u200b', inline: false },

        { 
            name: `🛡️ ${t('commands.settings.spamProtection')}`, 
            value: `└ ${t('commands.settings.spamDesc', { window: guildSettings.spamWindow, count: guildSettings.spamCount })}`, 
            inline: false 
        },
        ...
        { 
            name: `🚫 ${t('commands.settings.ignoredChannels')} (${ignoredChannelsRaw.length})`, 
            value: ignoredChannels, 
            inline: false 
        },
        { 
            name: `🚫 ${t('commands.settings.ignoredRoles')} (${ignoredRolesRaw.length})`, 
            value: ignoredRoles, 
            inline: false 
        }
      )
