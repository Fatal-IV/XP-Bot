...
// Liderlik tablosu (Gelişmiş JOIN sorgusu ile)
// database/sqlite.js içindeki getLeaderboard fonksiyonunu bununla değiştirin:

function getLeaderboard(guildId, type = 'all', limit = 10) {
  // GÜVENLİK: Limit kontrolü (Varsayılan 10)
  limit = parseInt(limit) || 10;

  if (type === 'all') {
    // users tablosundan direkt çekim (En hızlı yöntem)
    ...
    // Zaman aralıkları (Milisaniye cinsinden)
    if (type === 'day') timeThreshold = now - 24 * 60 * 60 * 1000;      // 1 Gün
    else if (type === 'week') timeThreshold = now - 7 * 24 * 60 * 60 * 1000; // 1 Hafta
    else if (type === 'month') timeThreshold = now - 30 * 24 * 60 * 60 * 1000; // 1 Ay
    else timeThreshold = 0; // Hata durumunda tüm zamanlar

    // JOIN Sorgusu: 
    // xp_history'den sıralamayı belirle, users tablosundan güncel Level ve XP bilgisini çek.
    // "this.db" YERİNE "db" KULLANIYORUZ.
    const stmt = db.prepare(`
      SELECT 
        u.user_id, 
        u.level, 
