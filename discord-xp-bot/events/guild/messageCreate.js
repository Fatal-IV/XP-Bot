    ...
    if (newLevel > oldLevel) {
      try {
        ...
        if (guildConfig.level_up_message) {
            // Veritabanında özel mesaj varsa onu kullan
            // DÜZELTME BURADA YAPILDI: {guild} değişimi eklendi
            levelUpMsg = guildConfig.level_up_message
                .replace(/{user}/g, message.author.toString())
                .replace(/{level}/g, newLevel)
                .replace(/{guild}/g, message.guild.name); // <-- EKLENEN SATIR
        } else {
            // Yoksa varsayılan dil dosyasını kullan
        ...
        // Eğer log kanalı varsa oraya, yoksa mesajın yazıldığı kanala at
